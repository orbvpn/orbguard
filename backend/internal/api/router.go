...
func (r *Router) Setup() http.Handler {
	...
	router.Group(func(pub chi.Router) {
		...
		pub.Get("/api/v1/stats/dashboard", r.handlers.Alerts.GetDashboard)

		// Auth endpoints (public - no auth required)
		pub.Post("/api/v1/auth/login", r.handlers.Auth.Login)
		pub.Post("/api/v1/auth/refresh", r.handlers.Auth.Refresh)
		pub.Post("/api/v1/auth/device", r.handlers.Auth.RegisterDevice)
	})
	...
	router.Route("/api/v1", func(api chi.Router) {
		...
		api.Route("/forensics", func(fr chi.Router) {
			// Capabilities and IOC stats
			fr.Get("/capabilities", r.handlers.Forensics.GetCapabilities)
			fr.Get("/iocs/stats", r.handlers.Forensics.GetIOCStats)

			// iOS Forensics (Flutter paths: /forensics/analyze/*)
			fr.Post("/analyze/shutdown-log", r.handlers.Forensics.AnalyzeShutdownLog)
			fr.Post("/analyze/backup", r.handlers.Forensics.AnalyzeBackup)
			fr.Post("/analyze/data-usage", r.handlers.Forensics.AnalyzeDataUsage)
			fr.Post("/analyze/sysdiagnose", r.handlers.Forensics.AnalyzeSysdiagnose)
			fr.Post("/analyze/logcat", r.handlers.Forensics.AnalyzeLogcat)
		...
		// Path aliases for Flutter compatibility
